...
async function ensureStatsColumns(): Promise<void> {
  ...
  try {
    ...
    if (!existing.has('years_experience')) {
      await pool.query("ALTER TABLE profile ADD COLUMN years_experience VARCHAR(50) DEFAULT '1+'");
    }
    if (!existing.has('projects_count')) {
      await pool.query("ALTER TABLE profile ADD COLUMN projects_count VARCHAR(50) DEFAULT '10+'");
    }
    ...
    if (profile.bio && typeof profile.bio === 'string' && profile.bio.includes('<!--STATS:')) {
      try {
        ...
        if (match && match[1]) {
          ...
          if (parsed.projects_count) profile.projects_count = parsed.projects_count;
          if (parsed.technologies_count) profile.technologies_count = parsed.technologies_count;
          if (parsed.repos_count) profile.repos_count = parsed.repos_count;
          if (parsed.badge_text) profile.badge_text = parsed.badge_text;
          if (parsed.greeting_text) profile.greeting_text = parsed.greeting_text;
        }
      } catch (err) {
  ...
  try {
    ...
    if (safeBio && safeBio.includes('<!--STATS:')) {
      try {
        ...
        if (match && match[1]) {
          const parsed = JSON.parse(match[1]);
          if (!safeYearsExp && parsed.years_experience) safeYearsExp = String(parsed.years_experience).trim();
          if (!safeProjectsCnt && parsed.projects_count) safeProjectsCnt = String(parsed.projects_count).trim();
          if (!safeTechCnt && parsed.technologies_count) safeTechCnt = String(parsed.technologies_count).trim();
          if (!safeReposCnt && parsed.repos_count) safeReposCnt = String(parsed.repos_count).trim();
        }
      } catch (err) {}
    }
